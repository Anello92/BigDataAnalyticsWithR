...
# Coletando os tweets uma vez conectado ao twitter
?searchTwitter

#######################################################################################

bigdata_tweets = searchTwitter("BigData", # Buscar um termo específico sobre "BigData"
                               n = 100,   # Retornar apenas 100 tweets
                               lang = "pt")

datascience_tweets = searchTwitter("DataScience", # Buscar um termo específico sobre "DataScience"
                                   n = 100,       # Retornar apenas 100 tweets
                                   lang = "pt") 


# Buscar a timeline de um usário específico 
...
# Convertendo os tweets para texto - percorrer o conjunto de ddados com sapply & getText()

textos_bigdata = sapply(bigdata_tweets,
                        function(x) x$getText())

head(bigdata_tweets)

textos_datascience = sapply(datascience_tweets, 
                            function(x) x$getText())
...
textos_bigdata_limpo <- limpaTweets(textos_bigdata_limpo) # a função se encontra em utils.R
...
# Converte para Corpus - trabalhar com funções próprias
